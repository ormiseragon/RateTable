...
st.caption(
    ...
    "此版 R_dut 為正交 yaw/pitch/roll 合成。"
)
...
with st.sidebar:
    ...
    st.header("DUT 相對於 Table (正交)")
    st.caption("dut_rz/ry/rx 為繞正交軸的 yaw/pitch/roll (ZYX 內旋)")
    dut_rz = st.slider("DUT yaw (deg)", -180, 180, 15, key="dut_rz")
    dut_ry = st.slider("DUT pitch (deg)", -180, 180, 10, key="dut_ry")
    dut_rx = st.slider("DUT roll (deg)", -180, 180, 5, key="dut_rx")
...
# DUT 相對於 Table: 正交 yaw/pitch/roll (ZYX 內旋, rz·ry·rx)
...
st.write(
    "- 此版 R_dut 為正交 yaw/pitch/roll 合成"
    " (R_dut = rotz(rz)·roty(ry)·rotx(rx))，dut_rz/ry/rx 為純 yaw/pitch/roll。"
)
st.write(
...
st.caption(
    "non-orthogonal 誤差反映在 Table 自身軸 (t_coor)，R_dut 為正交姿態，"
